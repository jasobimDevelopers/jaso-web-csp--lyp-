  ...
  <div>
    ...
    <div class="app-container">
      ...
      <el-dialog  :visible.sync="rectificationSheetFormVisible"
                  ...
                  :close-on-click-modal="false"
                  width="360px">
        <div slot="title" style="font-weight: bolder">新增质量整改单</div>
...
export default {
  ...
  methods: {
        ...
        if (valid) {
          ...
          addQualityCheck(partms).then((data) => {
            if (data) {
              ...
              getQualityCheckList({ projectId: this.listQuery.projectId, id: data.data.data, qualityType: 0 }).then((data2) => {
                if (data2) {
    ...
    rewardsPunishmentsDetails(item) {
      this.tableLine = item;
      getQualityCheckList({ projectId: this.listQuery.projectId, id: item.aboutId, qualityType: 0 }).then((data) => {
        if (data) {
